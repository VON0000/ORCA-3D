...
let parse_floats_from_string result =
  try Scanf.sscanf result "%f %f" (fun f1 f2 -> Geom.create_t f1 f2) with
  | Scanf.Scan_failure _ -> failwith "解析失败：输入字符串格式不正确"
  | End_of_file -> failwith "文件结束，无法解析"

let def_position id =
  let in_channel = open_in "initial_point" in
  let rec read_line current_line =
    let line = input_line in_channel in
    if current_line = id then line else read_line (current_line + 1)
  in
  let result = read_line 0 in
  close_in in_channel;
  parse_floats_from_string result

let def_dest id =
  let in_channel = open_in "initial_dest" in
  let rec read_line current_line =
    let line = input_line in_channel in
    if current_line = id then line else read_line (current_line + 1)
  in
...
let create id (exist_acft : t list ref) =
  let position = def_position id in
  let dest = def_dest id in
  let speed = def_speed position dest in
  let speedopt = def_speed position dest in
  let acft =
    {
  ...
  in
  (* Printf.printf "init_point %f %f\n" position.x position.y;
     Printf.printf "init_dest %f %f\n" dest.x dest.y; *)
  acft
